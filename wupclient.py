...
class wupclient:
    def __init__(self, ip='10.0.1.230', port=1337):
        try:
            ...
            print("ERROR: Couldn't connect to wupserver at", ip)

    def __del__(self):
        if self.fsa_handle is not None:
    ...
    def read(self, addr, count):
        data = struct.pack(">II", addr, count)
    ...
    def memcpy(self, dst, src, count):
        data = struct.pack(">III", dst, src, count)
    ...
    def MCP_CopyTitle(self, handle, path, dst_device_id, flush):
        inbuffer = make_buffer(0x27F)
        copy_string(inbuffer, path, 0x0)
        inbuffer2 = make_buffer(0x4)
        copy_word(inbuffer2, dst_device_id, 0x0)
        inbuffer3 = make_buffer(0x4)
    ...
    def mkdir(self, path, flags):
        ...
        if path[0] != "/":
            path = self.cwd + "/" + path
        ret = self.FSA_MakeDir(fsa_handle, path, flags)
        if ret == 0:
            return 0
        else:
